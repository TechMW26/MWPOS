...
export function AsmOrderPos({ mode = "asm" }: { mode?: "asm" | "cf" }) {
  ...
  useEffect(() => {
    let active = true;
    getJson<MarketplaceData>("/api/marketplace?storeType=DISTRIBUTOR", { ttlMs: 60_000, force: reloadKey > 0 })
      .then((payload) => {
        if (!active) return;
        const clean = {
  ...
  function addSelectedItem(pieces: number) {
    ...
    cart.addItem({
      ...
      quantity: pieces, unitPrice: Math.max(0, Math.round(Number(selectedSku.sellingPrice) || 0)),
  ...
  return (
    <div className="space-y-5 pb-24 md:pb-0">
      ...
      {step === "products" ? <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-4">{filteredSkus.map((sku) => <SkuProductCard key={sku.id} product={productsById.get(sku.productId)} sku={sku} inCart={cartBySku.get(sku.id)?.quantity || 0} onBuy={() => openQuantity(sku)} />)}</div>
        ...
        <Card><CardHeader className="flex-row items-center justify-between space-y-0"><div><CardTitle>Review order</CardTitle><p className="text-sm text-muted-foreground">For {selectedStore?.name} · {cart.totals.itemCount} pieces</p></div>{cart.items.length > 0 && <Button variant="ghost" size="sm" className="text-destructive" onClick={cart.clear}>Clear</Button>}</CardHeader><CardContent className="space-y-3">{cart.items.length === 0 ? <div className="py-12 text-center"><ShoppingCart className="mx-auto mb-3 h-10 w-10 text-muted-foreground/40" /><p className="font-medium">No items in this order</p><Button className="mt-4" onClick={() => setStep("products")}>Choose products</Button></div> : cart.items.map((item) => <div key={item.skuId} className="grid gap-3 rounded-lg border p-3 sm:grid-cols-[1fr_auto_auto] sm:items-center"><div className="min-w-0"><p className="truncate font-medium">{item.productName}</p><p className="text-xs text-muted-foreground">{getCartQuantityLabel(item)} · {item.sku} · {formatCurrency(item.unitPrice)} each</p></div><QuantityControl compact value={item.quantity} onChange={(quantity) => cart.setQuantity(item.skuId, quantity)} /><div className="flex items-center justify-between sm:w-24 sm:block sm:text-right"><span className="font-semibold">{formatCurrency(item.unitPrice * item.quantity)}</span><Button size="icon" variant="ghost" className="h-8 w-8 text-destructive sm:ml-auto" aria-label={`Remove ${item.productName}`} onClick={() => cart.removeItem(item.skuId)}><Trash2 className="h-4 w-4" /></Button></div></div>)}</CardContent></Card>
...
function AsmPosSkeleton() {
  return <div className="space-y-4"><Skeleton className="h-28 rounded-xl" /><Skeleton className="h-12 rounded-lg" /><Skeleton className="h-11" /><div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-4">{Array.from({ length: 6 }, (_, index) => <Skeleton key={index} className="h-72 rounded-[1.25rem]" />)}</div></div>;
}
