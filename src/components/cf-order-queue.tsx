...
export function CfOrderQueue() {
  ...
  async function load(force = false) { setLoading(true); const payload = await getJson<QueueOrder[]>(`/api/orders?days=365&limit=250${showAll ? "" : "&status=PENDING_CF_APPROVAL"}`, { ttlMs: 15_000, force }); setOrders(Array.isArray(payload) ? payload : []); setSelected(new Set()); setLoading(false); }
  ...
  async function approve() { setApproving(true); setMessage(""); const response = await fetch("/api/orders/bulk-approve", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderIds: Array.from(selected) }) }); const payload = await response.json(); setApproving(false); setMessage(response.ok ? `${payload.approved} order${payload.approved === 1 ? "" : "s"} approved.` : payload.message || "Approval failed."); invalidateJson("/api/orders"); invalidateJson("/api/dashboard"); await load(true); }
  return <div className="space-y-4">
    <div className="flex items-start justify-between gap-3 px-1"><div><h2 className="text-xl font-bold">Order queue</h2><p className="text-sm text-muted-foreground">Approve one or many assigned orders</p></div><button type="button" onClick={() => load(true)} className="flex h-11 w-11 items-center justify-center rounded-full bg-white shadow-sm" aria-label="Refresh"><RefreshCw className="h-4 w-4" /></button></div>
