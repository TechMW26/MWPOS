    ...
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>All Customer Stores ({stores.length})</CardTitle></CardHeader><CardContent>
        <DataTable data={stores} columns={[
          { key: 'logoUrl', header: 'Logo', render: (s:any) => s.logoUrl ? <img src={s.logoUrl} alt={s.name} className="h-9 w-9 rounded-md object-cover" /> : <span className="text-xs text-muted-foreground">—</span> },
          { key: 'name', header: 'Name' }, { key: 'city', header: 'City' }, { key: 'phone', header: 'Phone' },
          { key: 'ownerUid', header: 'Owner', render: (s:any) => s.ownerUid || <span className="text-xs text-muted-foreground">Unmapped</span> },
          { key: 'approvalStatus', header: 'Status', render: (s:any) => <Badge variant={s.approvalStatus === 'APPROVED' ? 'success' : 'warning'}>{s.approvalStatus}</Badge> },
        ]} />
      </CardContent></Card>
    </div>
  );
