...
export default function AdminCatalogPage() {
  ...
  const stores = storesLive.data;
  const loading = productsLive.loading || skusLive.loading || storesLive.loading;
  const error = productsLive.error || skusLive.error || storesLive.error;
  const live = productsLive.live && skusLive.live && storesLive.live;
  ...
  async function handleAssign(productId: string) {
    if (!selectedStore) { setAssignMsg('Select a store first'); return; }
    setAssignMsg('');
    try {
      await fetch('/api/catalog', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ productId, storeId: selectedStore }) });
      setAssignMsg('Product assigned to store');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm font-medium whitespace-nowrap">Assign to:</label>
          <select className="flex h-10 rounded-md border px-3 py-2 text-sm w-48" value={selectedStore} onChange={e => setSelectedStore(e.target.value)}>
